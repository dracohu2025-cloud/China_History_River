...
// FIX: Extract PodcastPin as independent component to avoid closure trap displacement bug
...
const PodcastPinComponent: React.FC<PodcastPinComponentProps> = ({
  ...
  // BUG FIX: Component will re-render, so screenX is always latest
  // Avoid using closure functions directly in parent map with outdated screenX
  ...
  return (
    ...
    >
      {/* PodcastPin container - optimized design */}
      <g transform={`translate(0, 0)`}>
        {/* Background rounded rectangle - gradient fill effect */}
        <rect 
          x={-22} 
          y={-14} 
          width={44} 
          ...
          strokeWidth={1.5}
        />
        {/* Top decoration bar - visual hierarchy */}
        <rect 
          x={-22} 
          y={-14} 
          width={44} 
          ...
          opacity={0.7}
        />
        {/* Book name - optimized typography */}
        {title && (
          <text 
            x={0} 
            y={-1} 
        ...
        {/* Year - different color distinction */}
        <text 
          x={0} 
          y={10} 
          fill="#64748b" 
        ...
        {/* Small dot decoration - implying podcast/audio */}
        <circle 
          cx={16} 
          cy={-5} 
          r={2} 
...
const RiverCanvas: React.FC<RiverCanvasProps> = ({ onEventSelect, onOpenEpisode, width, height }) => {
  ...
  // ===== DEBUGGING: Check 1949 event in KEY_EVENTS =====
  useEffect(() => {
    console.log('🔍 DEBUG Initialize RiverCanvas');
    
    // Check if 1949 exists in KEY_EVENTS
    const event1949 = KEY_EVENTS.find(e => e.year === 1949);
    if (event1949) {
      console.log('✅ 1949年事件在KEY_EVENTS中存在:', event1949);
    } else {
      console.error('❌ 1949年事件在KEY_EVENTS中不存在!');
      console.log('📊 KEY_EVENTS总数:', KEY_EVENTS.length);
      console.log('📊 KEY_EVENTS年份范围:', 
        Math.min(...KEY_EVENTS.map(e => e.year)), '到',
        Math.max(...KEY_EVENTS.map(e => e.year))
      );
    }
    
    // List all 1949-related events
    const all1949Events = KEY_EVENTS.filter(e => e.title.includes('1949') || e.year === 1949 || e.title.includes('新中'));
    console.log('📍 所有1949相关事件:', all1949Events);
  }, []);
  // =====================================================
  ...
  // ===== DEBUGGING: Check before data loading =====
  useEffect(() => {
    console.log('🔍 DEBUG: Podcast pin loading useEffect');
    const loadPins = async () => {
      try {
        console.log('🚀 Fetching /timeline-api/api/riverpins/');
        const response = await fetch('/timeline-api/api/riverpins/');
        console.log('📡 Response received:', response.status, response.statusText);
        const data = await response.json();
        console.log('📊 Data parsed:', data);
      } catch (error) {
        console.error('❌ Failed to load podcast pins:', error);
      }
    };
    loadPins();
  }, []);
  ...
  const eventLayoutNodes = useMemo(() => {
    // ===== DEBUGGING: Log all events before filtering =====
    console.log('🔍 DEBUG: All KEY_EVENTS:', KEY_EVENTS);
    console.log('🔍 DEBUG: KEY_EVENTS length:', KEY_EVENTS.length);
    
    const relevantEvents = KEY_EVENTS.filter(ev => {
      // Check 1949 specifically
      if (ev.year === 1949) {
        console.log('📍 DEBUG: 1949 event found in filter, importance:', ev.importance);
      }
      
      if (ev.importance === 1) {
        if (ev.year === 1949) console.log('✅ DEBUG: 1949 event passes filter (importance=1)');
        return true;
      }
      if (viewport.k <= 0.1) {
        if (ev.year === 1949) console.log('❌ DEBUG: 1949 event filtered (k <= 0.1)');
        return false;
      }
      if (viewport.k < 0.3 && ev.importance > 1) {
        if (ev.year === 1949) console.log('❌ DEBUG: 1949 event filtered (k < 0.3 and importance > 1)');
        return false;
      }
      if (viewport.k < 0.8 && ev.importance > 2) {
        if (ev.year === 1949) console.log('❌ DEBUG: 1949 event filtered (k < 0.8 and importance > 2)');
        return false;
      }
      if (viewport.k < 2.0 && ev.importance > 3) {
        if (ev.year === 1949) console.log('❌ DEBUG: 1949 event filtered (k < 2.0 and importance > 3)');
        return false;
      }
      if (viewport.k < 3.5 && ev.importance > 4) {
        if (ev.year === 1949) console.log('❌ DEBUG: 1949 event filtered (k < 3.5 and importance > 4)');
        return false;
      }
      if (viewport.k < 6.0 && ev.importance > 5) {
        if (ev.year === 1949) console.log('❌ DEBUG: 1949 event filtered (k < 6.0 and importance > 5)');
        return false;
      }
      return true;
    });
    
    console.log('🔍 DEBUG: Filtered relevantEvents:', relevantEvents);
    console.log('🔍 DEBUG: 1949 in relevantEvents:', relevantEvents.some(e => e.year === 1949));
    // =========================================================
    ...
    sortedEvents.forEach(ev => {
      // ===== DEBUGGING: Log 1949 event processing =====
      if (ev.year === 1949) {
        console.log('📍 DEBUG: Processing 1949 event for layout');
      }
      // =================================================
    ...
    // ===== DEBUGGING: Check if 1949 is in nodes =====
    const has1949InNodes = nodes.some(n => n.event.year === 1949);
    console.log('🔍 DEBUG: 1949 in layout nodes:', has1949InNodes);
    if (!has1949InNodes) {
      console.error('❌ ERROR: 1949 event NOT in layout nodes!');
      console.log('📊 Generated nodes:', nodes.map(n => ({year: n.event.year, title: n.event.title})));
    }
    // ===============================================
  ...
  // DEBUGGING: Check after eventLayoutNodes calculated
  useEffect(() => {
    console.log('🔍 DEBUG: eventLayoutNodes updated');
    console.log('🔍 DEBUG: eventLayoutNodes count:', eventLayoutNodes.length);
    
    const allYears = eventLayoutNodes.map(n => n.event.year).sort((a,b) => a - b);
    console.log('🔍 DEBUG: eventLayoutNodes years range:', 
      allYears[0], 'to', allYears[allYears.length - 1]);
    
    console.log('🔍 DEBUG: Contains 1949?', eventLayoutNodes.some(n => n.event.year === 1949));
  }, [eventLayoutNodes]);
    ...
    >
      <svg ref={svgRef} width={width} height={height} className="block">
        ...
        <g>
          {/* ===== 1949年专属轨道 (置顶显示) ===== */}
          {(() => {
            // Check if 1949 is in eventLayoutNodes
            const has1949InLayout = eventLayoutNodes.some(n => n.event.year === 1949);
            console.log('🔍 DEBUG: 1949 in eventLayoutNodes?', has1949InLayout);
            
            if (!has1949InLayout) {
              console.warn('⚠️ DEBUG: 1949 not found in eventLayoutNodes, showing external track');
              ...
              return (
                <g>
                  ...
                  <g transform={`translate(20, ${60 + 56 / 2})`}>
                    <text fill="#b91c1c" fontSize={12} fontWeight={700} textAnchor="start">1949年·新中国成立 (紧急显示)</text>
                    <line x1={0} y1={8} x2={180} y2={8} stroke="#b91c1c" strokeWidth={1} />
                  </g>
          ...
          {eventLayoutNodes.map((node) => {
            // ===== DEBUGGING: Log if this is 1949 =====
            if (node.event.year === 1949) {
              console.log('🔍 DEBUG: Rendering 1949 event node:', node);
            }
            // ==========================================
          ...
          {podcastPins.map((pin) => {
            ...
            const TRACK_HEIGHT = 56;
            const TRACK_MARGIN = 12;
            const title = pin.title || '';  // Get book name directly from pin
            const handleClick = (e: React.MouseEvent) => {
              e.stopPropagation();
              if (onOpenEpisode) onOpenEpisode(pin.jobId);
            };
            
            // FIX: Create a unique component to avoid closure trap
            // Key problem: onMouseEnter/onMouseLeave capture the screenX value at render time
            // When viewport changes, this value becomes outdated, but event handlers still use the old value
            
            // CRITICAL FIX: Add viewport dependency to key to force re-rendering as the timeline moves
            // Solves the bug where Pin position is out of sync when dragging
            return (
              <PodcastPinComponent
                key={`pin-${pin.jobId}-${viewport.x}-${viewport.k}`}  // ← Add viewport dependency, force re-render
                pin={pin}
                screenX={screenX}
                height={height}
